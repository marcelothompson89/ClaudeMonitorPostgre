...
async def scrape_cepal_eventos():
    """
    Scraper para la sección de actividades de CEPAL utilizando Playwright.
    """
    base_url = "https://www.cepal.org"
    url = "https://www.cepal.org/es/actividades"
    items = []
        ...
        try:
            ...
            for articulo in articulos:
                try:
                    ...
                    presentation_date = _parse_date_cepal_actividades(fecha_texto)
...
def _parse_date_cepal_actividades(date_str):
    """
    Parsear fecha de inicio de actividades CEPAL.
    Formatos posibles:
    - "20 Jul - 13 Sep 2026, Todo el día | Curso"
    - "3 Feb 2026 | Reunión"
    Retorna solo la fecha de inicio.
    """
    if not date_str:
        return None
    ...
    try:
        ...
        # Buscar el primer patrón día mes: "20 jul"
        pattern = r'(\d{1,2})\s+([a-záéíóú]+)'
...
if __name__ == "__main__":
    ...
    items = asyncio.run(scrape_cepal_actividades())

    # Imprimir resultados
    print(f"\nTotal de actividades encontradas: {len(items)}\n")
    for item in items:
        print(item)
        print("-" * 80)