...
async def scrape_alianza_pacifico_noticias():
    """
    Scraper para la sección de noticias de Alianza del Pacífico utilizando Playwright.
    """
    base_url = "https://alianzapacifico.net"
    url = "https://alianzapacifico.net/noticias/"
    items = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_selector("article.wp-show-posts-single", timeout=60000)
            ...
            articulos = soup.find_all("article", class_="wp-show-posts-single")

            for articulo in articulos:
                try:
                    # A) Extraer título y enlace
                    titulo_tag = articulo.find("h5", class_="wp-show-posts-entry-title")
                    ...
                    # B) Extraer fecha desde el elemento time
                    fecha_tag = articulo.find("time", class_="wp-show-posts-entry-date")
                    fecha_texto = fecha_tag.get_text(strip=True) if fecha_tag else None
                    presentation_date = _parse_date_alianza(fecha_texto)
                    ...
                    item = {
                        ...
                        'category': "Noticias",
                        'country': "Regional",
                        'institution': "Alianza del Pacífico",
...
def _parse_date_alianza(date_str):
    """
    Parsear una fecha en formato "30 enero, 2026" o similar.
    """
    if not date_str:
        return None
...
if __name__ == "__main__":
    ...
    items = asyncio.run(scrape_alianza_pacifico_noticias())
